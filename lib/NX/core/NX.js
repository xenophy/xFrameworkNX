...
NX.apply(NX, {
    ...
    /**
     * 繰り返し可能なオブジェクトの範囲を指定して、配列オブジェクトとして取得します。
     *
     * @param {Iterable} array 配列オブジェクトとして取得する、繰り返し可能なオブジェクト
    ...
    */
    ...
    service : function(o) {
        ...
        NX.iterate(o.items, function(cfg) {
            NX.ServiceMgr.register(NX.ServiceMgr.create(cfg, 'http'));
        });

        NX.ServiceMgr.all.each(function(srv) {
            srv.start();
        });
