...
NX.util.MarkDown = (function() {
    ...
    var stripLinkDefinitions = function(text) {
        ...
        var text = text.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+|\Z)/gm, function (wholeMatch,m1,m2,m3,m4) {
            m1 = m1.toLowerCase();
            url[m1] = encodeAmpsAndAngles(m2);
            if (m3) {
                return m3+m4;
            } else if (m4) {
                title[m1] = m4.replace(/"/g,"&quot;");
            }
    ...
    var hashHTMLBlocks = function(text) {
    ...
    var writeAnchorTag = function(wholeMatch,m1,m2,m3,m4,m5,m6,m7) {
        ...
        if (url == "") {
            ...
            if (url[link_id] != undefined) {
                url = url[link_id];
                if (title[link_id] != undefined) {
                    title = title[link_id];
                }
            }
            else {
    ...
    var encodeBackslashEscapes = function(text) {
